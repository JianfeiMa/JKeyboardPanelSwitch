...
public class ChattingResolvedActivity extends AppCompatActivity {
    ...
    private void assignViews() {
        ...
        if (actionBar != null) {
            actionBar.setSubtitle("actionBar0");
        }
        androidx.appcompat.app.ActionBar actionBar1 = getSupportActionBar();
        if (actionBar1 != null) {
            actionBar1.setSubtitle("actionBar1");
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_UP && event.getKeyCode() == KeyEvent.KEYCODE_BACK && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
        ...
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_BACK && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (mPanelRoot.isKeyboardShowing()) {
                KPSwitchConflictUtil.hidePanelAndKeyboard(mPanelRoot);
                return true;
            } else {
